...
**Snapshot-based comparison.** The comparison view operates on a snapshot of annotations taken at the Phase 2→3 transition. Revisions and new annotations created during Phase 3 appear in the student's own "My Annotations" tab and are stored in the database, but do not update other students' comparison views. This keeps the comparison stable as a discussion anchor — the verbal conversation is the real-time part. When the teacher advances to Phase 4, a fresh snapshot is taken that includes Phase 3 revisions, so students get credit for their evolved thinking in the Phase 4 comparison.
