...
async function main() {
  ...
  const testScenarioId = "ocean_plastic_campaign";
  ...
  if (researcher) {
    ...
    // Create test session
    const testSession = await prisma.classSession.upsert({
      where: { sessionCode: "TEST01" },
      update: {},
      create: {
        scenarioId: testScenarioId,
        teacherId: researcher.id,
        sessionCode: "TEST01",
        lifelineBudget: 6,
        locationHintCap: 5,
        characterHintCap: 2,
        perspectiveHintCap: 2,
        narrowedHintCap: 5,
        guidedFirstDetection: true,
      },
    });

    // Create test group with student
    const testGroup = await prisma.group.upsert({
      where: { groupId: "test-group-1" },
      update: {},
      create: {
    ...
    await prisma.groupMember.upsert({
      where: { userId_groupId: { userId: testStudent.id, groupId: testGroup.groupId } },
      update: {},
      create: {
    ...
    await prisma.studentActivity.upsert({
      where: { userId_sessionId: { userId: testStudent.id, sessionId: testSession.sessionId } },
      update: {},
      create: {
        userId: testStudent.id,
        sessionId: testSession.sessionId,
    ...
    console.log(`  Session code: ${testSession.sessionCode}, Student: ${testStudent.displayName}`);
